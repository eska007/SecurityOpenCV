...
public class DrawingPlane extends Canvas implements MouseListener, MouseMotionListener{
    ...
    public DrawingPlane() {
        gridArray = new int[Configuration.NUMBER_OF_COLS][Configuration.NUMBER_OF_ROWS];
    ...
    private boolean setGrid(int x, int y) {
        ...
        if(xIndex < 0 || xIndex >= Configuration.NUMBER_OF_ROWS) return false;
        if(yIndex < 0 || yIndex >= Configuration.NUMBER_OF_COLS) return false;
